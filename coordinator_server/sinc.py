...
class Sinc:
    ...
    def get_sinc_from(self, buffer):
        ...
        for log in logs:
            log_int = int(log)
            if log_int not in self.logs_dict:
                self.logs_dict[log_int] = logs[log]

            if len(self.logs_dict[log_int]) != len(logs[log]):
                if len(self.logs_dict[log_int]) < len(logs[log]):
                    self.logs_dict[log_int] = logs[log]
                else:
    ...
    def sinc_with_leader(self, buffer):
        ...
        hash = info["hash"]
        logs = info["logs_dict"]
        self.logs_dict = {int(key): value for key,value in logs.items()} 
        self.hash = hash 
